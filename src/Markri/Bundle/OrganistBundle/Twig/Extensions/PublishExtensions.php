...
namespace Markri\Bundle\OrganistBundle\Twig\Extensions;
...
class PublishExtensions extends Twig_Extension
{
    ...
    public function setContainer(ContainerInterface $container)
    {
        ...
        $twig->addGlobal('status_bitbucket', $container->getParameter('organist.bitbucket'));
        $twig->addGlobal('status_github', $container->getParameter('organist.github'));
    }
    ...
    public function renderMenu()
    {
        ...
        return $this->container->get('templating')->render('OrganistBundle::menu.html.twig', array(
    ...
    public function getApplicationSelect($id = null)
    {
        ...
        if(!empty($id)){
            $current = $em->getRepository('OrganistBundle:Application')->findOneById($id);
        }

        // Generate default route for first app (if present) and replace id with %s, to generate a route template for dashboard
        if(count($apps)){
            $url = $this->container->get('router')->generate('markri_organist_application_dashboard', array('application'=> $apps[0]->getId()), null);
            $url = str_replace($apps[0]->getId(), '%s', $url);
        }

        return $this->container->get('templating')->render('OrganistBundle:Application:select.html.twig', array(
            'dashboardurl'=> $url,
            'apps'=>$apps,
            'current'=>$current)
        );
    ...
    public function isApplicationCheckedOut($id)
    {
         ...
         */
        $application = $em->getRepository('OrganistBundle:Application')->findOneById($id);

...
use Markri\Bundle\OrganistBundle\Action\InitCommand;

use Markri\Bundle\OrganistBundle\Form\TargetEditType;
use Markri\Bundle\OrganistBundle\Form\TargetStep1Type;
use Markri\Bundle\OrganistBundle\Form\TargetStep2Type;
...
class TargetController extends Controller
{
    ...
    public function targetsAction(Application $application)
    {
        ...
        $targets = $em->getRepository('OrganistBundle:Target')->getOrderedByDTAP($application);
    ...
    public function detailAction(Target $target)
    {
        ...
        $logs = $em->getRepository('OrganistBundle:CommandLog')->getLogsForApplication($target->getApplication(), 50);
        $countLogs = $em->getRepository('OrganistBundle:CommandLog')->countLogsForTarget($target);
        $lastLog = $em->getRepository('OrganistBundle:CommandLog')->getLastDeployment($target);
    ...
    public function createStep1Action(Application $application)
    {
        ...
        if($request->getMethod() == 'POST'){
            ...
            if($formStep1->isValid()){
                ...
                return $this->redirect($this->generateUrl('markri_organist_target_createstep2', array('application' => $application->getId())));
            }
        }
    ...
    public function createStep2Action(Application $application)
    {
        ...
        /**
         * @var \Markri\Bundle\OrganistBundle\Entity\Environment $env
         */
        $env = $em->getRepository('OrganistBundle:Environment')->findOneById($envId);
        if(!$env){
        ...
        if($request->getMethod() == 'POST'){
            ...
            if($formStep2->isValid()){
                ...
                return $this->redirect($this->generateUrl('markri_organist_target_targets', array('application' => $application->getId())));
            }
        }
    ...
    public function editAction(Target $target)
    {
        ...
        if($request->getMethod() == 'POST'){
            ...
            if($form->isValid()){
                ...
                return $this->redirect($this->generateUrl('markri_organist_target_targets', array('application' => $target->getApplication()->getId())));
            }
        }
    ...
    public function deleteAction(Target $target)
    {
        ...
        return $this->redirect($this->generateUrl('markri_organist_target_targets', array('application' => $app->getId())));
    }
}

...
use Markri\Bundle\OrganistBundle\Form\ApplicationDeployType;
use Markri\Bundle\OrganistBundle\Form\ApplicationRollbackType;
use Markri\Bundle\OrganistBundle\Form\ApplicationSetupType;
use Markri\Bundle\OrganistBundle\Strategy\Commands\ActionFactory;
use Markri\Bundle\OrganistBundle\Strategy\Commands\CommandApplicationInterface;
use Markri\Bundle\OrganistBundle\Strategy\Commands\CommandTargetInterface;
use Markri\Bundle\OrganistBundle\Strategy\Strategy;
...
//        $commandFormFactory = $this->container->get('organist.commandformfactory');
//        $forms = array();
//
...
//
//                    // Render command
//                    $twig = $this->container->get('organist.twig.environment');
//                    $data = $form->getData();
...
//                        $em->flush();
//
//                        return $this->redirect($this->generateUrl('markri_organist_command_exec', array('commandlog' => $commandLog->getId())));
//                    }
//                }
//            }
//
...
//    }
    ...
    public function commandPanelAction(Application $application)
    {
         ...
         */
        $strategy = $this->container->get(strtolower($application->getDeploymentStrategy()));
        ...
        if($request->getMethod() == 'POST'){
            ...
            if ($request->request->has($deployForm->getName())){
                ...
                if($deployForm->isValid()){
                    ...
                    return $this->forward('OrganistBundle:Command:execTargetCommand', array(
    ...
    public function execAction(CommandLog $commandlog)
    {
        ...
        if($commandlog->getDatetimeEnd()){
            $this->get('session')->getFlashBag()->add('warning', sprintf('This command is already executed. <a href="%s" class="alert-link">Click here</a> if you want to re-execute it', $this->generateUrl('markri_organist_command_reexecute', array('commandlog'=>$commandlog->getId()))));
    ...
    {
        return array(
            'logs' => $this->getDoctrine()->getRepository('OrganistBundle:CommandLog')->getLogsForApplication($application),
            'application' => $application
        );
    }
    ...
    public function reExecuteAction(CommandLog $commandlog)
    {
        ...
        return $this->redirect($this->generateUrl('markri_organist_command_exec', array('commandlog' => $newCommand->getId())));
    }

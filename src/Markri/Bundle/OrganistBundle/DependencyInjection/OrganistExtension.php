...
namespace Markri\Bundle\OrganistBundle\DependencyInjection;
 ...
 */
class OrganistExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        $container->setParameter('organist.anyterm_user', $config['anyterm_user']);
        $container->setParameter('organist.anyterm_exec_port', $config['anyterm_exec_port']);
        $container->setParameter('organist.repositorypath', $config['repositorypath']);
        $container->setParameter('organist.bitbucket' , $config['externalstatus']['bitbucket']);
        $container->setParameter('organist.github' , $config['externalstatus']['github']);
    ...
    private function processApplicationTypes($applicationTypes)
    {
        ...
        foreach ($applicationTypes as $key => $params) {
            ...
            $definition = new Definition('Markri\Bundle\OrganistBundle\ApplicationType\ApplicationType');
            ...
            $containerKey = 'organist.type.' . $key;
        ...
        $this->containerBuilder->setParameter('organist.applicationTypeKeyLabels', $applicationTypeLabels);
    }
    ...
    private function processStrategies($strategies)
    {
        ...
        foreach ($strategies as $key => $params) {
            $definition = new Definition('Markri\Bundle\OrganistBundle\Strategy\Strategy');
            $definition->addMethodCall('setLabel', array($params['label']));
            $definition->addMethodCall('setKeyname', array($key));
        ...
        $this->containerBuilder->setParameter('organist.strategyKeyLabels', $strategyLabels);
    }
}

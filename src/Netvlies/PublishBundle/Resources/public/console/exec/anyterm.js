...
function handle_resp_error(resp) {
  if (resp.charAt(0)=="E") {
	...
	// for now just disable any message, maybe changit this to all but "subprocess terminated" msg
    //alert(msg);
    return true;
  }
  return false;
...
function create_img_button(imgfn,label,fn) {
  ...
  button_img.setAttributeNode(id_attr);
...
function create_term(elem_id,title,rows,cols,p,charset,scrollback) {
  ...
  //title_p.appendChild(create_img_button("close.gif","Close",close_term));
